% This file tries to generate the data from the paper
% Hoffmann: Benchmark Problem ? Nonlinear Control of a 3-DOF Robotic Manipulator
% 3DOF Problem
...
% generate Grid
qorig_bnd{1} = [ -135    10     ];%[ -45      45     ];
qorig_bnd{2} = [ -90      90     ];%[ -45      45     ];
qorig_bnd{3} = [  -100     45     ];

qbounds{1} = qorig_bnd{1}*pi/180; 
qbounds{2} = qorig_bnd{2}*pi/180; 
qbounds{3} = (qorig_bnd{3} + qorig_bnd{2})*pi/180; %-90??
qgriddensity = 8*pi/16;
...
%% Calculate Affine parameters theta = (theta1,...,theta16)
...
%clear NTHETAGrid THETAGrid DELTAGrid NDELTA NTHETA_hat
...
uNtheta01 = ureal('uNtheta01', 0); %? not sure if needed
...
%% Define the rational LFT model matrizes Ad,Bd,Cd,Dd in terms of theta
...
% i am certain that this is not needed (Michael)
%uTheta   = [];
%for ii = 1:ntheta
%    eval(['uTheta(', num2str(ii), ') = uthetaofdelta', num2strChars(ii, 2), ';']);
%end
...
% Define the uncertain state space system and perform model reduction
MRLFR_Ntheta = uss(A, B, C, D);
MRLFR_Ntheta = simplify(MRLFR_Ntheta, 'full'); %model reduction
...
%load data_to_prevent_rerunning.mat
%% Resubstitute 
...
save(['LPV_3DOF_controller_data_',thisdate,'.mat'],'WK','WR','WS','rho_bnd','theta_bnd','theta_mid','theta_min','theta_range','qdbounds','qbounds','MRLFR_Ntheta','KLFR_sim','dKLFR_sim');
