function [KLFR, CLLFR] = LFTLPV_OFsyn_SQLF_controller(argPLFR, argnu, argne, argKLFR, argCLLFR, argSDPoptions)
% -------------------------------------------------------------------------
% function : LFTLPV_OFsyn_SQLF_controller
% -------------------------------------------------------------------------
% Author   : Christian Hoffmann
% Version  : July, 16th 2013
% Copyright: 2013
% -------------------------------------------------------------------------
% Syntax   : [KLFR, CLLFR] = LFTLPV_OFsyn_SQLF_controller ...
%                                     (argPLFR                      , ...
%                                      argCLLFR                     , ...
%                                      nu                           , ...
%                                      ny                           , ...
...
Sizes.nwKth  = argCLLFR.Sizes.nwKth;
Sizes.nzKth  = argCLLFR.Sizes.nzKth;
...
%% Evaluate solution
    ...
    CLLFR.CompTimeK   =  solution.solvertime;
    CLLFR.DecVarsK    =  max(getvariables(Constraints));
    
    CLLFR.LMISizeK     =  0;
    LMIids = getlmiid(Constraints);
    for ii = 1:numel(LMIids)
        CLLFR.LMISizeK = CLLFR.LMISizeK + size(double(Constraints(LMIids(ii))), 1);
    end
    CLLFR.GammaK      =  gopt;
end % function