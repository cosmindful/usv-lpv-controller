...
% LPVSPLIT  Extract PSS data based on independent variable range
%
% B = LPVSPLIT(A,NAME1,RANGE1,NAME2,RANGE2,....) extracts data from the PSS
...
% the PSS A on the domain specified by the NAME/INDEX pairs. Each INDEX
...
% from the PSS A on the domain specified by the NAME/VALUES pairs. VALUES 
...
% an RGRID object. This extracts data from the PSS A based on the
% independent variables and data ranges in DOMAIN.
%
% See also: lpvinterp.

% Check whether we have index or value call:
nin = nargin;
cflag = 1;
...
% Find locations of NAMEs in the PSS list of IVs
...
if niv==0
    ...
    out = pss(m.DataPrivate(INDEX{:}),rgrid(ivn,ivd,ivrb));
end
