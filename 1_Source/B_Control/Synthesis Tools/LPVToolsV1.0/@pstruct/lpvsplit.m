...
% LPVSPLIT  Extract PSTRUCT data based on independent variable range
%
% B = LPVSPLIT(A,NAME1,RANGE1,NAME2,RANGE2,....) extracts data from the PSTRUCT
...
% the PSTRUCT A on the domain specified by the NAME/INDEX pairs. Each INDEX
...
% from the PSTRUCT A on the domain specified by the NAME/VALUES pairs. VALUES 
...
% an RGRID object. This extracts data from the PSTRUCT A based on the
...
% Find locations of NAMEs in the PSTRUCT list of IVs
...
if niv==0
    ...
    if cflag==1
        % Split functionality by value/range
    ...
    out = pstruct(m.DataPrivate(INDEX{:}),rgrid(ivn,ivd,ivrb));
end
