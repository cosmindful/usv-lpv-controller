...
% LPVSPLIT  Extract PFRD data based on independent variable range
%
% B = LPVSPLIT(A,NAME1,RANGE1,NAME2,RANGE2,....) extracts data from the PFRD
...
% the PFRD A on the domain specified by the NAME/INDEX pairs. Each INDEX
...
% from the PFRD A on the domain specified by the NAME/VALUES pairs. VALUES 
...
% an RGRID object. This extracts data from the PFRD A based on the
...
% Find locations of NAMEs in the PFRD list of IVs
...
if niv==0
    ...
    out = pfrd(m.DataPrivate(INDEX{:}),rgrid(ivn,ivd,ivrb));
end
